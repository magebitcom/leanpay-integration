...
### Promotion vendorProductCode ignored for configurable product variants

For configurable products, the checkout promotion resolution (`Helper/Data.php::validateProduct()`) only read the Leanpay Promotions attributes from the top-level order item. A promotion configured on the purchased variant (simple child) was never seen, so the customer was redirected to Leanpay with the default financial product instead of the promotion's custom `vendorProductCode`.

**Fixed:** Checkout now resolves the promotion from the purchased variant first and falls back to the configurable parent. If both are configured, the variant wins. Works for both order and quote flows. Simple products, category-level and global promotions are unchanged.

### Time-based promotion end date was excluded

*"Date If Time-based To (same day included)"* was compared against midnight at the start of the end day, so promotions silently stopped one day early.

**Fixed:** Time-based promotions now run through the full end day (product and category paths).

### Time-based promotions evaluated in GMT

The current time was taken in GMT instead of the store timezone, shifting promotion start and end by the store's UTC offset.

**Fixed:** Time-based promotions are evaluated in the store's configured timezone.

### Exclusive mode check

Fixed an operator-precedence bug in the exclusive-mode "all cart items share the same promotion code" check.

### Behaviour change for all merchants

Time-based promotions now include the full end day and follow store-local time. This matches the admin field labels, but merchants who compensated for the old behaviour (for example by setting the "To" date one day later) should revert that adjustment after updating.

### Testing Recommendations

1. Promotion set on the **simple variant only**: buy the variant, payload must contain the promo `vendorProductCode`
2. Promotion set on the **configurable parent only**: promo code still sent (regression)
3. Different codes on parent and variant: the **variant's** code wins
4. Promotion on a standalone simple product: promo code sent
5. "To" date = today: promotion active the whole day; "To" = yesterday: default product
6. "From" = tomorrow: default product
7. Store timezone differs from server: promotion follows the store's date
8. Exclusive mode, promoted variant plus an unrelated product in the cart: default product (by design)
9. Category-level promotion: applied, end date inclusive
10. Non-time-based promotion: always applied
11. Complete a payment on the Leanpay stage gateway end to end
